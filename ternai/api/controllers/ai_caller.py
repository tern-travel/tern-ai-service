...
import json
from api.models import ExternalCall, APICall, Prompt
...
def response_validation(source_prompt:Prompt, response_text:str):
  ...
  try: 
    ...
    return False, "Invalid JSON Returned from OpenAI. Could not load into Dictionary."   
  ...
  if set(req_keys).issubset(set(given_keys)) == False:
    #We're missing keys
    return False, "Missing Keys. Required keys were not provided from OpenAI."   
